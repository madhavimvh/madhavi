...
def paying_debt_off_in_a_year(balance, a_interest_rate):
    """# In short:
    ...
    m_interest_rate = (a_interest_rate) / 12.0
    m_payment_l_b = balance / 12
    m_payment_u_b = (balance * (1 + m_interest_rate)**12) / 12.0
    new_bal = balance
    epsilon = 0.0001
    guess = (m_payment_l_b + m_payment_u_b)/2
    while True:
        month = 1
        while month <= 12:
            new_bal = new_bal - guess
            new_bal = new_bal + (m_interest_rate * new_bal)
            month += 1
